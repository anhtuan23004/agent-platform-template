...
| `work-item add` | Task specification | Create a hierarchical `spec`, `epic`, `story`, `spike`, or `task` record. | `--id`, `--type`, `--title`, optional `--parent`, `--scope`, `--out-of-scope`, `--optional`, `--acceptance` |
| `work-item update` | Task state | Update a work item's parent, status, scope, optional work, acceptance criteria, evidence, or docs. | `--id`, optional update fields |
...
| `trace` | Observability | Record an agent execution trace and print trace quality. | `--summary`, optional trace fields including `--work-item` and `--parent-trace` |
| `failure add` | Failure attribution | Link a normalized failure pattern to a trace. | `--trace`, `--signature`, `--category`, `--summary`, `--causal` |
| `failure cluster` | Failure attribution | Group failure patterns by signature and category. | none |
...
| `eval report` | Verification | Show recorded eval run results. | none |
| `proposal promote` | Entropy auditing | Promote a proposal after eval gates pass. | `--id` |
| `query matrix` | Task state | Show durable story proof matrix. | optional `--numeric` |
| `query work-items` | Task state | Show hierarchical spec, epic, story, spike, and task records. | optional `--parent` |
...
| `query failures` | Failure attribution | Show normalized failure attribution records. | optional `--trace` |
| `query proposals` | Entropy auditing | Show committed Self-Harness proposals. | none |
| `query eval-cases` | Verification | Show registered eval cases. | none |
| `query eval-runs` | Verification | Show recorded eval run results. | none |
| `query trace-tree` | Observability | Show a parent trace and direct child traces. | trace id |
