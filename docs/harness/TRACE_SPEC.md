...
| `task_summary` | TEXT | Yes | One sentence, at least 10 characters, naming the outcome or attempted outcome. | `Updated trace documentation and validation evidence` |
...
| `work_item_id` | TEXT | Standard+ for bounded work item execution | Work item id from `work_item`. Required for delegated child traces. | `T001` |
| `parent_trace_id` | INTEGER | Required for sub-agent traces | Parent conductor trace id. Leave null for top-level traces. | `42` |
| `agent` | TEXT | Optional for minimal; Standard+ expected | Short agent/tool name. | `codex` |
| `actions_taken` | TEXT | Standard+ | JSON array text. With the current CLI, pass a comma-separated list and the CLI stores JSON text. | `["read docs/harness/HARNESS.md","updated docs/harness/TRACE_SPEC.md"]` |
| `files_read` | TEXT | Standard+ | JSON array text of paths or command names. With the current CLI, pass a comma-separated list. | `["docs/harness/HARNESS.md","scripts/bin/harness-cli query matrix"]` |
...
| `decisions_made` | TEXT | Detailed | JSON array text of decision strings. Include scope decisions, validation choices, and explicit non-goals. | `["Kept this change docs-only; installer propagation remains out of scope"]` |
...
| `harness_friction` | TEXT | Standard+ when friction exists; Detailed always | Free text naming what was hard, missing, ambiguous, or repeated. Use `none` only when the agent actively checked and found no friction. | `New docs are not in installer copy list; recorded as out-of-scope follow-up.` |
...
and should match the story packet / validation evidence. This repo does not
maintain a separate `docs/decisions/` tree.
...
```text
New docs are not copied by scripts/install-harness.sh, but installer
propagation is out of scope for this docs-only task.
```
...
```bash
scripts/bin/harness-cli trace \
  --summary "Updated trace specification and Harness reference" \
  --intake 36 \
  --story US-004 \
  --agent codex \
  --outcome completed \
  --actions "read HARNESS docs,drafted TRACE_SPEC.md,updated HARNESS.md,ran rg checks" \
  --read "docs/harness/HARNESS.md,scripts/schema/001-init.sql" \
...
Use `harness_friction` for narrative trace friction. Use failure attribution
when the same mechanism should be queryable, clustered, or used by
...
scripts/bin/harness-cli failure add \
  --trace 42 \
  --signature "context rules missed schema decision" \
  --category context-gap \
  --summary "Schema edits did not require the durable layer decision." \
  --causal root \
  --verifier-grounded 1
```

Use `failure cluster` to see repeated mechanisms before creating or reviewing
Self-Harness proposals.
