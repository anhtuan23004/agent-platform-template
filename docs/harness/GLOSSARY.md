...
is closed. `story verify <id>` executes a story's `verify_command`,
`story verify-all` runs all configured story proof commands, and traces can
warn when linked story verification has not passed.
...
It lets agents discover available commands, arguments, responsibilities, and
...
which compares a trace's recorded reads against compiled context rules and
retrieval triggers.
